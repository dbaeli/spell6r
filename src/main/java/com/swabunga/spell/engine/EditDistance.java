...
/**
 * This class is based on Levenshtein Distance algorithms, and it calculates how similar two words are. If the words are
 * identical, then the distance is 0. The more that the words have in common, the lower the distance value. The distance
 * value is based on how many operations it takes to get from one word to the other. Possible operations are swapping
 * characters, adding a character, deleting a character, and substituting a character. The resulting distance is the sum
 * of these operations weighted by their cost, which can be set in the Configuration object. When there are multiple
 * ways to convert one word into the other, the lowest cost distance is returned. <br/>
 * Another way to think about this: what are the cheapest operations that would have to be done on the "original" word
 * to end up with the "similar" word? Each operation has a cost, and these are added up to get the distance. <br/>
 * 
...
public class EditDistance {
   ...
   * @return a number representing how easy or complex it is to transform on word into a similar one.
   */
  ...
  public static final int getDistance(String word, String similar, int[][] matrix) {
    /*
     * JMH Again, there is no need to have a global class matrix variable in this class. I have removed it and made the
     * getDistance static final DMV: I refactored this method to make it more efficient, more readable, and simpler. I
     * also fixed a bug with how the distance was being calculated. You could get wrong distances if you compared ("abc"
     * to "ab") depending on what you had setup your COST_REMOVE_CHAR and EDIT_INSERTION_COST values to - that is now
     * fixed. WRS: I added a distance for case comparison, so a misspelling of "i" would be closer to "I" than to "a".
     */
    ...
    for (i = 1; i != a_size; ++i) {
      ...
      for (j = 1; j != b_size; ++j) {
        ...
        if (sourceChar == otherChar) {
          matrix[i][j] = matrix[i - 1][j - 1]; // no change required, so just carry the current
          // cost up
          continue;
        }
  ...
  /**
   * For debugging, this creates a string that represents the matrix. To read the matrix, look at any square. That is
   * the cost to get from the partial letters along the top to the partial letters along the side.
   * 
