...
/**
 * Container for various methods that any <code>SpellDictionary</code> will use. This class is based on the original
 * Jazzy aspell port.
 * <p/>
 * Derived classes will need words list files as spell checking reference. Words list file is a dictionary with one word
 * per line. There are many open source dictionary files, see: <a href="http://wordlist.sourceforge.net/">
 * http://wordlist.sourceforge.net/</a>
 * <p/>
 * You can choose words lists form <a href="http://aspell.net/">aspell</a> many differents languages dictionaries. To
 * grab some, install <code>aspell</code> and the dictionaries you require. Then run aspell specifying the name of the
 ...
 * </pre>
 * 
 * Note: the number following the language is the size indicator. A bigger number gives a more extensive language
 * coverage. Size 40 is more than adequate for many usages.
 * <p/>
 * For some languages, Aspell can also supply you with the phonetic file. On Windows, go into aspell <code>data</code>
 * directory and copy the phonetic file corresponding to your language, for example the <code>fr_phonet.dat</code> for
 * the <code>fr</code> language. The phonetic file should be in directory <code>/usr/share/aspell</code> on Unix.
 * 
 * @see GenericTransformator GenericTransformator for information on phonetic files.
 */
public abstract class SpellDictionaryASpell implements SpellDictionary {
   ...
   * 
   * @param phonetic The file to use for phonetic transformation of the words list. If <code>phonetic</code> is null,
   *          the the transformation uses {@link DoubleMeta} transformation.
   ...
   * Constructs a new SpellDictionaryASpell
   * 
   * @param phonetic The Reader to use for phonetic transformation of the words list. If <code>phonetic</code> is null,
   *          the the transformation uses {@link DoubleMeta} transformation.
   * @throws java.io.IOException indicates problems reading the phonetic information
   */
   ...
   * @param word Suggestions for given misspelt word
   * @param threshold The lower boundary of similarity to misspelt word
   * @param matrix Two dimensional int array used to calculate edit distance. Allocating this memory outside of the
   *          function will greatly improve efficiency.
   * @return Vector a List of suggestions
   */
  ...
  /**
   * When we don't come up with any suggestions (probably because the threshold was too strict), then pick the best
   * guesses from the those words that have the same phonetic code.
   * <p>
   * This method is only needed to provide backward compatibility.
   * 
   ...
   * 
   * @param word - the word we are trying spell correct
   * @param matrix Two dimensional array of int used to calculate edit distance. Allocating this memory outside of the
   *          function will greatly improve efficiency.
   * @param wordList - the linked list that will get the best guess
   */
