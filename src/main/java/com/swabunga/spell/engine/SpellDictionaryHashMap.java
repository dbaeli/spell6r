...
public class SpellDictionaryHashMap extends SpellDictionaryASpell {
  /**
   * A field indicating the initial hash map capacity (16KB) for the main dictionary hash map. Interested to see what
   * the performance of a smaller initial capacity is like.
   */
  private final static int INITIAL_CAPACITY = 16 * 1024;

  /**
   * The hashmap that contains the word dictionary. The map is hashed on the doublemeta code. The map entry contains a
   * LinkedList of words that have the same double meta code.
   */
   ...
   * 
   * @param wordList The file containing the words list for the dictionary
   * @throws java.io.FileNotFoundException indicates problems locating the words list file on the system
   * @throws java.io.IOException indicates problems reading the words list file
   */
   ...
   * @throws java.io.FileNotFoundException indicates problems locating the file on the system
   * @throws java.io.IOException indicates problems reading the words list file
   */
  ...
  /**
   * Dictionary constructor that uses an aspell phonetic file to build the transformation table. Encoding is used for
   * phonetic file only; default encoding is used for wordList
   * 
   * @param wordList The file containing the words list for the dictionary
   * @param phonetic The file to use for phonetic transformation of the wordlist.
   ...
   * 
   * @param wordList The file containing the words list for the dictionary
   * @param phonetic The reader to use for phonetic transformation of the wordlist.
   * @throws java.io.IOException indicates problems reading the words list or phonetic information
   */
  ...
  /**
   * Add words from a file to existing dictionary hashmap. This function can be called as many times as needed to build
   * the internal word list. Duplicates are not added.
   * <p>
   * Note that adding a dictionary does not affect the target dictionary file for the addWord method. That is, addWord()
   * continues to make additions to the dictionary file specified in createDictionary()
   * <P>
   * 
  ...
  /**
   * Add words from a Reader to existing dictionary hashmap. This function can be called as many times as needed to
   ...
   * <p>
   * This is a very slow function. On my machine it takes quite a while to load the data in. I suspect that we could
   * speed this up quite allot.
   */
  ...
  /**
   * Adds to the existing dictionary from a word list file. If the word already exists in the dictionary, a new entry is
   * not added.
   * <p>
   * Each word in the reader should be on a separate line.
   * <p>
   * Note: for whatever reason that I haven't yet looked into, the phonetic codes for a particular word map to a vector
   * of words rather than a hash table. This is a drag since in order to check for duplicates you have to iterate
   * through all the words that use the phonetic code. If the vector-based implementation is important, it may be better
  ...
  /**
   * Allocates a word, if it is not already present in the dictionary. A word with a different case is considered the
   * same.
   * 
   * @param word The word to add
   */
