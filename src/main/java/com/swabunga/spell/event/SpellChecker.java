 ...
 * <p/>
 * By default, the class makes a user dictionary to accumulate added words. Since this user directory has no file assign
 * to persist added words, they will be retained for the duration of the spell checker instance. If you set a user
 * dictionary like {@link com.swabunga.spell.engine.SpellDictionaryHashMap SpellDictionaryHashMap} to persist the added
 * word, the user dictionary will have the possibility to grow and be available across differents invocations of the
 * spell checker.
 * 
...
public class SpellChecker {
  ...
  /**
   * Accumulates a dictionary at the end of the dictionaries list used for looking up words. Adding a dictionary give
   * the flexibility to assign the base language dictionary, then a more technical, then...
   * 
   * @param dictionary the dictionary to add at the end of the dictionary list.
   */
  public void loadDictionary(SpellDictionary dictionary) {
    if (dictionary == null) {
  ...
  /**
   * This method clears the words that are currently being remembered as <code>Ignore All</code> words and
   * <code>Replace All</code> words.
   */
  public void reset() {
  ...
  /**
   * Verifies if the word that is being spell checked contains at least a digit. Returns true if this word contains a
   * digit.
   * 
  ...
  /**
   * Verifies if the word that is being spell checked contains an Internet address. The method look for typical protocol
   * or the habitual string in the word:
   * <ul>
   * <li>http://</li>
   * <li>ftp://</li>
   * <li>https://</li>
   ...
   * </ul>
   * 
   * One limitation is that this method cannot currently recognize email addresses. Since the 'word' that is passed in,
   * may in fact contain the rest of the document to be checked, it is not (yet!) a good idea to scan for the @
   * character.
   * 
  ...
  /**
   * Verifies if the word that is being spell checked contains lower and upper cased characters. Note that a phrase
   * beginning with an upper cased character is not considered a mixed case word.
   * 
   ...
   * <ul>
   * <li>The default user dictionary or the one set through {@link SpellChecker#setUserDictionary}</li>
   * <li>The dictionary specified at construction time, if any.</li>
   * <li>Any dictionary in the order they were added through {@link SpellChecker#addDictionary}</li>
   * </ul>
   * 
  ...
  /**
   * Produces a list of suggested word after looking for suggestions in various dictionaries. The order of dictionary
   * lookup is:
   * <ul>
   * <li>The default user dictionary or the one set through {@link SpellChecker#setUserDictionary}</li>
   * <li>The dictionary specified at construction time, if any.</li>
  ...
  public final int checkSpelling(WordTokenizer tokenizer) {
    return checkSpelling(tokenizer, true);
  }

  public final int checkSpelling(WordTokenizer tokenizer, boolean suggest) {
    return checkSpelling(tokenizer, new HashSet(), suggest);
  }

  /**
   * String check, with a default StringWordTokenizer
   * 
   * @param sentence
   * @param suggest
   * @return
   */
  public final int checkSpelling(String sentence, boolean suggest) {
    StringWordTokenizer wt = new StringWordTokenizer(sentence);
    return this.checkSpelling(wt, suggest);
  }

  public final int checkSpelling(File file, boolean suggest) {
    FileWordTokenizer fwt = new FileWordTokenizer(file);
    return this.checkSpelling(fwt, suggest);
  }
   ...
   * 
   * @param tokenizer The media containing the text to analyze.
   * @param suggest true if the suggestions have to be computed
   * @return Either SPELLCHECK_OK, SPELLCHECK_CANCEL or the number of errors found. The number of errors are those that
   *         are found BEFORE any corrections are made.
   */
  // DBA-FIX-200609: Implement checkSpelling without suggestions (fast checking).
  public final int checkSpelling(WordTokenizer tokenizer, Set missspeltWords, boolean suggest) {
        ...
        if ((config.getBoolean(Configuration.SPELL_IGNOREMIXEDCASE) && isMixedCaseWord(word, tokenizer.isNewSentence()))
          ...
          if (!isIgnored(word)) {
            ...
            if (autoReplaceWords.containsKey(word)) {
              ...
              // DBA-FIX-200609: Implement checkSpelling without suggestions (fast checking).
              if (suggest) {
                suggestions = getSuggestions(word, config.getInteger(Configuration.SPELL_THRESHOLD));
    ...
    else if (missspeltWords.size() == 0)
      return SPELLCHECK_OK;
    else
      return missspeltWords.size();
  }
   ...
   */
  // DBA-FIX-200609: Implement checkSpelling without suggestions (fast checking).
  public final int countSpelling(WordTokenizer tokenizer) {
    int errors = 0;
    boolean terminated = false;
    // Keep track of the previous word
    // String previousWord = null;
  ...
  private boolean capitalizeSuggestions(String word, WordTokenizer wordTokenizer) {
    // if SPELL_IGNORESENTENCECAPITALIZATION and the initial word is capitalized, suggestions should also
    // be capitalized
