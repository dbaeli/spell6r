...
/**
 * This class tokenizes a swing document model. It also allows for the document model to be changed when corrections
 * occur.
 * 
...
public class DocumentWordTokenizer implements WordTokenizer {
  ...
  /**
   * Is this a special case where the currentWordStart, currntWordEnd and nextWordPos have already been calculated. (see
   * nextWord)
   */
  ...
  public DocumentWordTokenizer(Document document) {
    ...
    try {
      ...
      // DBA-FIX-200609: Delete first char of the document lead to ArrayIndex errors.
      // [Explanation: Segments does not always start from 0]
  ...
  private static int getNextWordStart(Segment text, int startPos) {
    ...
    // DBA-FIX-200609: Delete first char of the document lead to ArrayIndex errors.
    if (startPos >= text.getBeginIndex() && startPos <= text.getEndIndex())
  ...
  /**
   * Sets the current word position at the start of the word containing the char at position pos. This way a call to
   * nextWord() will return this word.
   * 
  ...
  public void replaceWord(String newWord) {
    ...
    if (currentWordPos != -1) {
      try {
        ...
        // DBA-FIX: use the attr to keep previous word style
