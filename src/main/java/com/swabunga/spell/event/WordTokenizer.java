...
/**
 * <p>
 * An interface for objects which take a text-based media as input, and iterate through the words in the text stored in
 * that media. Examples of such media could be Strings, Documents, Files, TextComponents etc.
 * </P>
 * 
 * <P>
 * When the object is instantiated, and before the first call to <CODE>next()</CODE> is made, the following methods
 * should throw a <CODE>WordNotFoundException</CODE>:<br>
 * <CODE>getCurrentWordEnd()</CODE>, <CODE>getCurrentWordPosition()</CODE>, <CODE>isNewSentence()</CODE> and
 * <CODE>replaceWord()</CODE>.
 * </P>
 * 
 * <P>
 * A call to <CODE>next()</CODE> when <CODE>hasMoreWords()</CODE> returns false should throw a
 * <CODE>WordNotFoundException</CODE>.
 * </P>
 * 
...
public interface WordTokenizer {
  ...
  /**
   * This returns the next word in the iteration. Note that any implementation should return the current word, and then
   * replace the current word with the next word found in the input text (if one exists).
   * 
   ...
   * 
   * <p/>
   * When a word is replaced care should be taken that the WordTokenizer repositions itself such that the words that
   * were added aren't rechecked. Of course this is not mandatory, maybe there is a case when an application doesn't
   * need to do this.
   * <p/>
   * 
