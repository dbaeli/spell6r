        ...
        for (i = 0; i < MAX_ROOTS; i++) {
            if (roots[i] != null) {
                ...
                while (itGuess.hasNext()) {
                    GuessWord glstK = (GuessWord) itGuess.next();
    ...
    private int replchars(List wlst, String word) {
        ...
        for (int i = 0; i < numrep; i++) {
            ...
            while ((r = word.indexOf(reptable[i].pattern, r)) != -1) {
                candidate = word.substring(0, r) + reptable[i].replacement + word.substring(r + lenp);

                if (!wlst.contains(candidate) && check(candidate)) {
                    if (ns < maxSug)
                        wlst.add(new Word(candidate, 0));
                    else
                        return wlst.size();
                }
        ...
        for (q = 0; q < wl; q++) {
            for (int i = 0; i < ctryl; i++) {
                ...
                if (!wlst.contains(candidateStr) && check(candidateStr)) {
                    ...
                    if (ns < maxSug)
                        wlst.add(new Word(candidateStr, 0));
                    else
                        return ns;
                }
            }
        ...
        for (p = 1; (p + 1) < (wl + 1); p++) {
            ...
            if (check(candidateStr)) {
                ...
                if (check(candidateStr)) {
                    ...
                    if (ns < maxSug)
                        wlst.add(new Word(new String(candidate), 0));
                    else
                        return ns;
                }
            }
